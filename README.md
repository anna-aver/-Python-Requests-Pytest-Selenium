...
**Задачи:** <br> 
1. Автоматизировать часть проверок регресса с помощью Pytest, Request и Selenium. Результаты обработать в Allure Report.
2. Автоматизировать основной пользовательский сценарий.
...
|--------|---------------------------------------------|---------------------|
| 1      | Проверка статуса ответа на запрос GET/pokemons (получение списка покемонов по ID тренера) | response status code = 200|
| 2      | Проверка части ответа на запрос GET/pokemons (наличие поля name сзаданным значением в ответе) |в ответе в json приходит массив объектов data с корректным полем name|
| 3      | Проверить, что запрос GET /pokemons возвращает ответ заданной структуры с помощью параметризации |в ответе в json приходит массив объектов data c  корректными полями name, trainer_id, id|
...
| 1.1      | - Корректная почта<br> - Корректный пароль | Успешная авторизация, сравнение id тренера|
