...
Задача - автоматизировать часть проверок регресса с помощью Pytest, Request и Selenium. Результаты обработать в Allure Report.
...
2	Проверка части ответа на метод GET/pokemons (наличие поля name с заданным значением в ответе)	в ответе в json приходит массив объектов data с корректным полем name
...
-	Корректная почта
-	Корректный пароль	Успешная авторизация, сравнение id тренера
2	Негативные проверки:
2.1	-	Некорректная почта
-	Корректный пароль	Введите корректную почту
2.2	-	Корректная почта
-	Некорректный пароль	Неверные логин или пароль
2.3	-	Некорректная почта
-	Некорректный пароль	Введите корректную почту
2.4	-	Пустая почта
-	Корректный пароль	Введите почту
2.5	-	Корректная почта
-	Пустой пароль	Введите пароль

Файл main.py:
Автоматизировано прохождение основного сценария тренажера: битва покемонов.
Детали реализации:
...
4.	Если остался один живой покемон, создать еще четырех (максимально разрешенное количество живых покемонов – 5)
...
b.	твой покемон победил – отвязать покемона от покебола, вывести информацию о завершении битвы
...
-	Создать виртуальное окружение внутри папки проекта: python –m venv env 
-	Активировать окружение:  .\env\Scripts\activate
-	Установка необходимых библиотек: pip install request pytest selenium allure-pytest 
-	Подключение allure :
o	 cd <путь до каталога\ allure-2.19.0\bin\
o	.\allure serve <путь до каталога с результатами>
-	Заполнение необходимых персональных данных в файле info/default_data.py
-	Запуск автотестов либо через мензурку, либо через терминал: pytest tests/test_pokemon.py
-	Ожидаемый результат: получим отчет о прохождении тестов.

Автор
Анна Чередникова (@AnnaC)
